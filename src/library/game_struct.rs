...
impl GameStart {
    ...
    pub fn new() -> GameStart {
        ...
        money: 0,
        exhange_rate: 1,
        game_time: TimeValues { months: 1, year: -50_000, century: -500},
        }
    }
...
pub impl TimeValues {
    pub fn pass_time(&mut self) {
        ...
        if (self.months == 12 ) {
            ...
            println!("Your time is before time itself how did this happen? month:{}", self.months);
            self.months = 1;
        } else {
            self.months += 1;
        }
...
/// Function that return's the raw time value.
pub fn get_time(&self) -> i32 {
    self.time
}

/// Function that print's out in readable format the amount of time that has passed since game
/// start.
pub fn print_formated_time(&self) {
